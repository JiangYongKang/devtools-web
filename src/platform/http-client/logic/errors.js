...
function extractCauseChain(error, maxDepth = MAX_CAUSE_CHAIN_LENGTH) {
  ...
  while (current && depth < maxDepth) {
    const causeInfo = {
      message: truncateString(current.message || String(current), MAX_ERROR_MESSAGE_LENGTH),
...
function wrapError(originalError, errorCode, context = {}) {
  if (originalError && originalError.errorCode && originalError.diagnostic) {
...
export {
    createError, createSerializableDiagnostic, extractCauseChain, getErrorMessage, isHttpClientError,
    toSerializable, truncateString, wrapError
}
