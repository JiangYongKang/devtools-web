...
const UNICODE_BLOCKS = [
  ...
  { start: 0x0860, end: 0x086F, name: 'Arabic Extended-B' },
  { start: 0x0870, end: 0x089F, name: 'Arabic Extended-A' },
  { start: 0x08A0, end: 0x08FF, name: 'Arabic Extended-C' },
  ...
  { start: 0x10A80, end: 0x10AAF, name: 'Old North Arabian' },
  ...
  { start: 0x10E60, end: 0x10E7F, name: 'Yezidi' },
  { start: 0x10E80, end: 0x10EBF, name: 'Arabic Extended-D' },
  ...
  { start: 0x11800, end: 0x1183F, name: 'Dogra' },
  ...
  { start: 0x11AC0, end: 0x11AEF, name: 'Pau Cin Hau' },
  ...
  { start: 0x11F60, end: 0x11FAF, name: 'Nag Mundari' },
  ...
  { start: 0x13430, end: 0x1343F, name: 'Egyptian Hieroglyph Format Controls' },
  ...
  { start: 0x1E000, end: 0x1E02F, name: 'Latin Extended-G' },
  { start: 0x1E100, end: 0x1E1DF, name: 'Nyiakeng Puachue Hmong' },
  { start: 0x1E2C0, end: 0x1E2FF, name: 'Wancho' },
  { start: 0x1E4D0, end: 0x1E4FF, name: 'Nag Mundari' },
  { start: 0x1E7E0, end: 0x1E7FF, name: 'Cherokee' },
...
function getBlockName(codePoint) {
...
function inferCategoryFromBlock(blockName, codePoint) {
  ...
  if (blockName) {
    if (blockName.includes('Surrogates')) {
      return 'Cs'
    }
    if (blockName.includes('Private Use')) {
      return 'Co'
    }
  }
...
function getGlyphPlaceholder(codePoint) {
  if (isControlCharacter(codePoint)) {
    const controlNames = {
...
function getCodePointName(codePoint, blockName) {
  ...
  if (codePoint >= 0x3400 && codePoint <= 0x4DBF) {
    return 'CJK Unified Ideograph Extension A'
  }
  
  if (codePoint >= 0x20000 && codePoint <= 0x2A6DF) {
    return 'CJK Unified Ideograph Extension B'
  }
  
  if (codePoint >= 0x2A700 && codePoint <= 0x2B73F) {
    return 'CJK Unified Ideograph Extension C'
  }
  
  if (codePoint >= 0x2B740 && codePoint <= 0x2B81F) {
    return 'CJK Unified Ideograph Extension D'
  }
  
  if (codePoint >= 0x2B820 && codePoint <= 0x2CEAF) {
    return 'CJK Unified Ideograph Extension E'
  }
  
  if (codePoint >= 0x2CEB0 && codePoint <= 0x2EBEF) {
    return 'CJK Unified Ideograph Extension F'
  }
  
  if (codePoint >= 0x30000 && codePoint <= 0x3134F) {
    return 'CJK Unified Ideograph Extension G'
  }
  
  if (codePoint >= 0x31350 && codePoint <= 0x323AF) {
    return 'CJK Unified Ideograph Extension H'
  }
  ...
  if (codePoint >= 0xDB80 && codePoint <= 0xDBFF) {
    return 'High Private Use Surrogate'
  }
...
class UnicodeDataManager {
  constructor() {
    this.cache = new Map()
  }
  
  getProperties(codePoint) {
    const cacheKey = codePoint
    ...
    const category = inferCategoryFromBlock(block, codePoint)
    const name = getCodePointName(codePoint, block)
    const glyph = getGlyphPlaceholder(codePoint)
  ...
  async loadProperties(codePoints) {
    return codePoints.map((codePoint) => {
      return {
        ...this.getProperties(codePoint),
      }
    })
  }
}

const unicodeDataManager = new UnicodeDataManager()

function getUnicodeProperties(codePoint) {
  return unicodeDataManager.getProperties(codePoint)
}

async function loadUnicodeProperties(codePoints) {
  return unicodeDataManager.loadProperties(codePoints)
}
